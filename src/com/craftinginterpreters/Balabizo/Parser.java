...
class Parser {
  ...
  private Stmt classDeclaration() {
    Token name = consume(IDENTIFIER, "Balabizo, Expect class name.");
    consume(LEFT_BRACE, "Balabizo, Expect '{' before class body.");
    ...
    consume(RIGHT_BRACE, "Balabizo, Expect '}' after class body.");
  ...
  private Stmt breakStatement() {
    consume(SEMICOLON, "Balabizo Expect ';' after 'break'.");
    ...
    while (true) { 
      if (match(LEFT_PAREN)) {
        ...
        Token name = consume(IDENTIFIER,
            "Balabizo, Expect property name after '.'.");
    ...
    if (!check(RIGHT_PAREN)) {
      do {
        if (arguments.size() >= 255) {
          error(peek(), "Balabizo, Can't have more than 255 arguments.");
        }
